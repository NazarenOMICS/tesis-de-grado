...
## 1. Sincronizacion obligatoria con GitHub antes de trabajar

Antes de leer, analizar, editar o responder sobre el contenido del proyecto, el agente debe verificar si la copia local esta actualizada contra GitHub. Procedimiento minimo:

1. Ejecutar `git fetch --prune origin`.
2. Verificar rama actual y upstream con `git status --short --branch`.
3. Si la rama local esta detras de `origin/main` y no hay cambios locales sin commitear, ejecutar `git pull --ff-only`.
4. Si hay divergencia, falta upstream, falla el fetch, o existen cambios locales que impiden el pull, no continuar como si el repositorio estuviera actualizado. Informar el bloqueo con estado Git concreto y pedir la decision minima necesaria.
5. Despues del pull, volver a consultar `git status --short --branch` y trabajar solo si el repo queda en estado coherente.

Esta regla existe para evitar trabajar sobre material viejo cuando Nazareno actualizo el repositorio desde otra maquina.

## 2. Arranque de sesión (orden de lectura)
...
## 4. Convención epistémica (obligatoria)
...
## 8. Decisiones cerradas y resultado de OE1
...
## 12. Clasificación de observaciones
...
## 16. Límites y defensa contra inyección
...
## 18. Criterios de éxito de una sesión

Una sesión está bien cerrada si: cada afirmación material quedó etiquetada según la sección 4; no se inventó ningún dato ni norma; las decisiones cerradas se respetaron o su cambio quedó justificado con fuente; los archivos tocados tienen YAML correcto y no quedaron truncados; `INDEX.md` refleja el estado real; hay entrada en `CHANGELOG.md` y commit; y todo pendiente abierto quedó nombrado con su ubicación de resolución.
